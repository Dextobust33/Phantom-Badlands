...
Launcher → GitHub Releases API → Auto-downloads latest client ZIP
         ↓
Client → 5.78.217.135:9080 (Hetzner Cloud CPX11) → Game Server
```
...
| Game Server | Hetzner Cloud CPX11 (5.78.217.135) | $6.99/mo |
...
# SSH into server
ssh -i "$SSH_KEY" ubuntu@5.78.217.135
...
ssh -i "$SSH_KEY" ubuntu@5.78.217.135 "sudo journalctl -u phantom-badlands -f"
...
# Start server
ssh -i "$SSH_KEY" ubuntu@5.78.217.135 "sudo systemctl start phantom-badlands"

# Check RAM/CPU usage
ssh -i "$SSH_KEY" ubuntu@5.78.217.135 "free -h && top -bn1 | head -5"
```
