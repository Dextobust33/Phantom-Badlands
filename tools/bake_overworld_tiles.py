...
def _signature(im):
    """A picture's fingerprint: size, colour, and the SILHOUETTE over a 4x4 grid.

    ⛑ NOT A HASH, AND NOT A PLAIN COLOUR AVERAGE - both were tried and both are wrong here. A
    hash cannot survive Godot's import at all. A colour average does not survive it either:
    measured across 113 tiles, 88 came back outside a tolerance of 8, for two separate reasons.
    Fully transparent pixels keep their RGB in a PNG and lose it on import (birdbath: 432 of its
    1024 pixels are transparent-but-coloured), and the imported texture may be VRAM-compressed,
    which shifts colour on its own (anvil, which has no transparent-with-colour pixels at all,
    moved by 18 levels).

    ALPHA is what survives exactly - it matched on every one of those 88 - and alpha over a grid
    is the SILHOUETTE, which is the thing that actually differs between two different pictures. A
    cut-off sword, a striped wall banner and a round stone fountain have nothing in common there.
    Colour is kept as well, alpha-PREMULTIPLIED so a transparent pixel contributes nothing
    whatever its stored RGB, with a loose tolerance that only catches a repaint."""
    w, h = im.size
    px = im.load()
    cells = []
    for gy in range(4):
        for gx in range(4):
            x0 = w * gx // 4
            x1 = max(x0 + 1, w * (gx + 1) // 4)
            y0 = h * gy // 4
            y1 = max(y0 + 1, h * (gy + 1) // 4)
            tot = 0
            n = 0
    ...
    for y in range(h):
        for x in range(w):
            ...
            pb += b * a // 255
    n = max(1, w * h)
    return [w, h, pr // n, pg // n, pb // n] + cells
...
def write_art_fingerprints(out_dir):
    ...
    for kind in ('ground', 'tile', 'overlay', 'big'):
        ...
        for f in sorted(_os.listdir(d)):
            ...
            rows.append(['%s/%s' % (kind, f[:-4])] + _signature(im))
    lines = [
        '## GENERATED by tools/bake_overworld_tiles.py - DO NOT HAND-EDIT.',
        '##',
    ...
    for row in rows:
        lines.append('	"%s": [%s],' % (row[0], ', '.join(str(v) for v in row[1:])))
    lines.append('}')
    lines.append('')
