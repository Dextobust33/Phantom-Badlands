...
===== FOCUSED vs RANDOM GEAR (%d fights/cell) =====" % N)
	...
	print("average = no class kit at all (what every prior class measurement used)")
	print("kit     = average PLUS the Hoarder kit for that archetype, affixes unsorted")
	print("focus   = the same kit, sifted for the stats that class actually wants")
	print("%-11s %-8s %s" % ["Class", "Path", "L30 elite: avg    kit    focus  |  L80 elite: avg    kit    focus"])
	for c in ALL_CLASSES:
		var row := "%-11s %-8s" % [c[0], c[1]]
		for lvl in [30, 80]:
			var cell := ""
			for g in ["average", "kit", "focus"]:
				var wins := 0
				var turns := 0
				var casts := 0
				for i in range(N):
...
func _apply_class_kit(ch, klass: String, glevel: int, best_of: int) -> void:
...
func make_char(level: int, gear: String, klass: String = "Fighter", race: String = "Human"):
	...
	if gear in ["kit", "focus", "focus_epic"]:
		_apply_class_kit(ch, klass, glevel, FOCUS_ROLLS if gear != "kit" else 1)
