...
===== WHAT NERF DOES A GROWN PLAYER NEED? (target 60% at a normal fight) =====")
	...
	print(head)
	_grow_immortal = true
	for klass in ["Barbarian", "Paladin", "Sorcerer", "Sage"]:
		for lvl in LEVELS:
			var pooled: Array = []
			for m in MULTS:
				pooled.append(0)
...
func run_trkprobe() -> void:
	for klass in ["Grifter", "Ranger", "Ninja"]:
		...
		for f in range(fights):
			...
			while t < 60 and ch.current_hp > 0 and int(monster.get("current_hp", 0)) > 0 and not combat.get("combat_ended", false):
				...
				var mhp0: int = int(monster.get("current_hp", 0))
				var e0: int = ch.current_energy
				_player_act_trickster(combat, ch)
				# infer what was cast: energy fell and one hand card is gone
				var newhand: Array = combat.get("combat_hand", [])
				# Infer from the HAND, not from energy: a successful cast consumes its card, and
				# an energy-delta test scored 76-83% basic attacks against a truthful counter
				# that says 94-98% casts. Instrument first.
				var played := "attack"
				for c in hand:
					if not newhand.has(c):
						played = String(c); break
				casts[played] = int(casts.get(played, 0)) + 1
				dmg_total += maxi(0, mhp0 - int(monster.get("current_hp", 0)))
				_monster_turn_if_owed(combat)
			turns_total += t
		var keys: Array = casts.keys()
		keys.sort()
		var line := ""
		for k in keys:
			line += "%s=%.2f  " % [k, float(casts[k]) / maxf(1.0, float(turns_total))]
		print("
%-8s  %.1f turns/fight, %d dmg/turn" % [klass, float(turns_total) / float(fights), int(float(dmg_total) / maxf(1.0, float(turns_total)))])
		print("   per-turn cast rate: %s" % line)
