...
func _render_page() -> void:
	...
	match _current_page:
		...
		"world":
			...
			_add_separator()
			_subtitle_subline("Hunting grounds MOVE every 3 hours, so their coordinates cannot be written down. This finds the current nearest one and puts you one step outside it.")
			_add_button("Take Me To The Nearest Hunting Ground", "gm_find_hotzone", Color(1.0, 0.68, 0.2))
			_add_separator()
